...
1. **Create the feature manifest** at `.github/status/feature.json` with agent assignments and task breakdown. The manifest's `initialTaskCount` field **MUST be the count of tasks the parser will recognise in tasks.md**, NOT a hand-counted estimate. To get this value, run the dashboard's built-in self-test before writing it: `pwsh -NoProfile -File <your-dashboard-loop-script> -SelfTest` parses the tasks.md, prints phase/task counts, and exits 0 on success. Use the reported `Total tasks` line as `initialTaskCount`. Do NOT proceed if `-SelfTest` returns nonzero:
...
2. Start the dashboard monitor by running `<your-dashboard-loop-script>` as a background process.
